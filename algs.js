...
function pushForward(board, color) {
    let moves = getAllLegalMoves(board, color);
...
function aggressive(board, color) {
    let moves = getAllLegalMoves(board, color);
    let direction = color === WHITE ? 1 : -1;
    let movesDescribed = [];
    for (let i = 0; i < moves.length; i++) {
...
function aggressivePlus(board, color) {
    let moves = getAllLegalMoves(board, color);
    let direction = color === WHITE ? 1 : -1;
    let movesDescribed = [];
    for (let i = 0; i < moves.length; i++) {
        ...
        if (isCheckmate(newBoard, oppColor(color))) {
            ...
            if (move.to[1] === 3 || move.to[1] === 4)
                centerControlBefore += 0.5;
            let centerControl = 0;
            if (distTravelled > 0) {
...
function blockOpponent(board, color) {
    let moves = getAllLegalMoves(board, color);
...
function defensive(board, color) {
    let moves = getAllLegalMoves(board, color);
...
function throwAsap(board, color) {
    let moves = getAllLegalMoves(board, color);
    let direction = color === WHITE ? 1 : -1;
    let movesDescribed = [];
    for (let i = 0; i < moves.length; i++) {
...
function kingRace(board, color) {
    ...
    if ((color === WHITE && kingPos[0] === 7) || (color === BLACK && kingPos[0] === 0)) {
        let noKingBackwardsMoves = moves.filter((move) => move.piece.type !== KING || move.to[0] - move.from[0] !== -direction);
        if (noKingBackwardsMoves.length > 0) {
            return noKingBackwardsMoves[Math.floor(Math.random() * noKingBackwardsMoves.length)];
        }
        return moves[Math.floor(Math.random() * moves.length)];
    }
    let advanceKingMoves = [];
    for (let move of moves) {
